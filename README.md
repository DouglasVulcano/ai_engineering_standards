...
> Padrão único de engenharia para agentes de IA — empacotado como uma **skill centralizada do
> Claude Code**. Uma fonte da verdade, aplicada de forma consistente por qualquer agente de
> qualquer modelo, em qualquer projeto.

![Claude Code Skill](https://img.shields.io/badge/Claude%20Code-Skill-6C4BF6)
![Scope](https://img.shields.io/badge/scope-global%20%7C%20per--project-informational)
![Lang](https://img.shields.io/badge/docs-PT--BR-green)
...
Este repositório transforma a minha configuração pessoal de IA (`prompts.txt` + `skills.txt`) em um
...
2. **Skill `engineering-standards`** — o mesmo conhecimento empacotado para o Claude Code, com
   *progressive disclosure* (carrega só o domínio relevante à tarefa → máxima performance de contexto).
3. **`install-skill.sh`** — o comando que importa tudo como skill **centralizada** em `~/.claude/skills/`
   e registra o slash command `/standards`.
...
| # | Pilar | Resumo |
|---|---|---|
| 1 | **Workflow & Governança** | *Issue-first, PR-driven*. Toda tarefa (Correção/Melhoria/Nova função) começa numa **Issue**; todo deploy passa por um **PR que menciona a Issue** (`Closes #N`). Conventional Commits. Alimenta o `CLAUDE.md`/`AGENTS.md` do projeto. |
| 2 | **Motion & UI/UX** | Toda interface tem **skeleton, lazy loading e animações de entrada/saída/carregamento/progresso**. Honra `prefers-reduced-motion`; anima só `transform`/`opacity`. Frequency Gate + Web Interface Guidelines. |
| 3 | **Observabilidade + Qualidade + Testes** | OpenTelemetry (base) + Sentry/Datadog/New Relic · Biome, contratos de arquitetura, Commitlint, Knip, Stryker · unit + integração + E2E (Playwright) com cobertura no Codecov. |
| 4 | **Arsenal (MCP & Skills)** | Ferramentas certas para cada tarefa: shadcn-ui-mcp, 21st.dev Magic, chrome-devtools-mcp, design-motion-principles, web-design-guidelines, humanizer. |
...
├── ai-engineering-standards.md        # ⭐ especificação completa (fonte única da verdade)
...
│   ├── SKILL.md                       # hub enxuto (dispara sozinho pelos gatilhos)
...
```bash
git clone https://github.com/<seu-usuario>/ai-engineering-standards.git ~/ai_config
bash ~/ai_config/install-skill.sh
```

O instalador é **idempotente**: copia a skill para `~/.claude/skills/engineering-standards/`,
empacota a spec completa em `references/` e cria o comando `/standards`. Reabra o Claude Code (ou
rode `/skills`) e pronto.

**Escopo por projeto** (em vez de global), instalando dentro de um repo específico:
```bash
...
- **Automático** — a skill dispara sozinha quando o seu pedido casa com os gatilhos: *"crie a
  issue/PR"*, *"revise a UI"*, *"adicione skeleton/lazy loading"*, *"configure observabilidade"*,
  *"siga os padrões"* etc.
- **Explícito** — o slash command:
  ...
  /standards testes     # só qualidade/testes
  /standards arsenal    # escolher/instalar um MCP ou skill
  ```
...
**Claude Code (CLI/IDE)** — clone o repo e rode `install-skill.sh` (como acima). Para atualizar:
```bash
cd ~/ai_config && git pull && bash install-skill.sh
```

**Sem git** — gere um pacote (`tar -czf ai-standards.tar.gz -C ~/ai_config .`), leve para a outra
máquina, extraia em `~/ai_config` e rode o instalador.

**Claude Desktop / claude.ai (web)** — não usam `~/.claude`. Instale pela UI de *Skills* fazendo
upload da pasta `engineering-standards/` (compacte-a). O slash command `/standards` é exclusivo do
Claude Code e não se aplica lá.

> ⚠️ **Não viajam automaticamente:** os MCP servers com API key (21st.dev, shadcn, chrome-devtools)
...
## Editar / evoluir o padrão

1. Edite os arquivos-fonte em `ai_config/` (o `ai-engineering-standards.md` e/ou a skill).
2. Reinstale: `bash install-skill.sh`.
3. Commit + push. Nas outras máquinas: `git pull && bash install-skill.sh`.
